...
const AddCamera = () => {
  ...
  const validateIP = (ip: string) => {
    ...
    return ip
      .split(".")
      .every((part) => parseInt(part) >= 0 && parseInt(part) <= 255);
  };
  ...
  const validate = () => {
    ...
    else if (!validateIP(form.ipAddress))
      ...
      <div className="bg-slate-800 rounded-lg p-6 border border-slate-700 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            />
            {errors.ipAddress && (
              <p className="text-red-400 text-xs mt-1">{errors.ipAddress}</p>
            )}
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, frameRate: Number(e.target.value) }))
              }
        ...
        <div className="flex items-center space-x-3 pt-2">
          <input
            ...
            onChange={(e) =>
              setForm((p) => ({ ...p, recording: e.target.checked }))
            }
            className="w-4 h-4 text-blue-600 rounded"
          />
          <label htmlFor="recording" className="text-sm text-slate-400">
            Enable Recording
          </label>
        </div>
      </div>
